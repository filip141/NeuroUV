	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		if (event.getAction() == MotionEvent.ACTION_UP) {
			...
			if (onJoystickMoveListener != null){
				...
				onJoystickMoveListener.onValueChanged(getYValue(),getXValue(), 
						getDirection());}
		}
		if (onJoystickMoveListener != null
...
//				onJoystickMoveListener.onValueChanged(getAngle(),getPower(), getDirection());
				onJoystickMoveListener.onValueChanged(getYValue(),getXValue(), 
						getDirection());
//				onJoystickMoveListener.onValueChanged(yPosition, xPosition, getDirection());
			}
		}
		return true;
	}
	...
	private int getXValue() {
		double y = -100*((centerX - xPosition)/(joystickRadius));
	return (int) y;
		
	}
	
	private int getYValue() {
		double y = 100*((centerY - yPosition)/(joystickRadius));
		return (int) y;
	}
		...
		while (!Thread.interrupted()) {
			post(new Runnable() {
				public void run() {
					if (onJoystickMoveListener != null)
//						onJoystickMoveListener.onValueChanged(getAngle(),getPower(), getDirection());
						onJoystickMoveListener.onValueChanged(getYValue(),getXValue(), getDirection());
				}
			});
			try {
